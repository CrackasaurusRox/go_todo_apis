...
func (ts *taskServer) tagHandler(w http.ResponseWriter, req *http.Request) {
	log.Printf("handing tasks by tag at %s\n", req.URL.Path)
...
func (ts *taskServer) dueHandler(w http.ResponseWriter, req *http.Request) {
	log.Printf("handing tasks by due at %s\n", req.URL.Path)
...
func main() {
	...
	mux.HandleFunc("/tag/", server.taskHandler)
	mux.HandleFunc("/due/", server.taskHandler)
